...
const FEEDBACK_TAGS = new Set(["好搭", "难搭", "想看外套搭配", "缺少搭档", "缺少鞋子", "暂时少穿", "想多穿", "适合购买同类"]);
const OUTFIT_FEEDBACK_TAGS = new Set(["值得保留", "不想要这类", "想看类似", "太正式", "太常规", "比例不对", "厚重冲突", "颜色不对", "需要换鞋", "需要换外套", "需要换内搭", "想看短裤搭配", "想要更简单"]);
...
export function wardrobeImportApi(options = {}) {
  ...
  async function loadOutfits() {
    ...
    catch (error) { if (error.code === "ENOENT") return { version: 1, outfits: [] }; throw error; }
  }

  async function loadOutfitFeedback() {
    try { return JSON.parse(await readFile(outfitFeedbackFile, "utf8")); }
    catch (error) { if (error.code === "ENOENT") return {}; throw error; }
  }

  async function outfitAssetVersion() {
    const files = await readdir(outfitAssetDir, { withFileTypes: true });
    const timestamps = await Promise.all([
      stat(outfitsFile).then((entry) => entry.mtimeMs).catch(() => 0),
      ...files
        .filter((entry) => entry.isFile())
        .map((entry) => stat(path.join(outfitAssetDir, entry.name)).then((file) => file.mtimeMs).catch(() => 0)),
    ]);
    return Math.trunc(Math.max(...timestamps, 0)).toString(36);
  }
  ...
  async function handler(req, res, next) {
    ...
    try {
      ...
      if (url.pathname === "/api/import/outfits" && req.method === "GET") {
        const manifest = await loadOutfits();
        return json(res, 200, { ...manifest, assetVersion: await outfitAssetVersion() });
      }
      if (url.pathname === "/api/import/outfit-feedback" && req.method === "GET") {
        return json(res, 200, await loadOutfitFeedback());
      }
      const outfitFeedbackMatch = url.pathname.match(/^\/api\/import\/outfit-feedback\/([\w.-]+)$/i);
      if (outfitFeedbackMatch && req.method === "PATCH") {
        const id = outfitFeedbackMatch[1];
        const input = await body(req, 256 * 1024);
        if (input.rating !== undefined && (!Number.isInteger(input.rating) || input.rating < 0 || input.rating > 5)) return json(res, 400, { error: "rating must be an integer from 0 to 5" });
        if (input.feedbackTags !== undefined && (!Array.isArray(input.feedbackTags) || input.feedbackTags.some((tag) => typeof tag !== "string" || !OUTFIT_FEEDBACK_TAGS.has(tag)))) return json(res, 400, { error: "feedbackTags contains an unsupported tag" });
        if (input.feedbackNote !== undefined && input.feedbackNote !== null && typeof input.feedbackNote !== "string") return json(res, 400, { error: "feedbackNote must be a string or null" });
        const feedback = await loadOutfitFeedback();
        const current = feedback[id] || {};
        const updated = {
          rating: input.rating === undefined ? Number.isInteger(current.rating) ? current.rating : 0 : input.rating,
          feedbackTags: input.feedbackTags === undefined ? Array.isArray(current.feedbackTags) ? current.feedbackTags : [] : [...new Set(input.feedbackTags)].slice(0, 8),
          feedbackNote: input.feedbackNote === undefined ? current.feedbackNote || "" : input.feedbackNote?.trim().slice(0, 500) || "",
          updatedAt: new Date().toISOString(),
        };
        await atomicJson(outfitFeedbackFile, { ...feedback, [id]: updated });
      ...
      if (wardrobeDeleteMatch && req.method === "PATCH") {
        ...
        if (input.name !== undefined && (typeof input.name !== "string" || !input.name.trim())) return json(res, 400, { error: "name must be a non-empty string" });
        if (input.part !== undefined && !PARTS.has(input.part)) return json(res, 400, { error: "invalid part" });
        if (input.color !== undefined && (typeof input.color !== "string" || !HEX_COLOR.test(input.color))) return json(res, 400, { error: "color must be a six-digit hex value" });
        if (input.secondaryColor !== undefined && input.secondaryColor !== null && (typeof input.secondaryColor !== "string" || !HEX_COLOR.test(input.secondaryColor))) return json(res, 400, { error: "secondaryColor must be null or a six-digit hex value" });
        if (input.colorName !== undefined && input.colorName !== null && typeof input.colorName !== "string") return json(res, 400, { error: "colorName must be a string or null" });
        if (input.rating !== undefined && (!Number.isInteger(input.rating) || input.rating < 0 || input.rating > 5)) return json(res, 400, { error: "rating must be an integer from 0 to 5" });
        if (input.feedbackTags !== undefined && (!Array.isArray(input.feedbackTags) || input.feedbackTags.some((tag) => typeof tag !== "string" || !FEEDBACK_TAGS.has(tag)))) return json(res, 400, { error: "feedbackTags contains an unsupported tag" });
        if (input.feedbackNote !== undefined && input.feedbackNote !== null && typeof input.feedbackNote !== "string") return json(res, 400, { error: "feedbackNote must be a string or null" });
        const current = records[index];
        const updated = {
          ...current,
          name: input.name === undefined ? current.name : input.name.trim().slice(0, 120),
          part: input.part === undefined ? current.part : input.part,
          color: input.color === undefined ? current.color : input.color.toLowerCase(),
          secondaryColor: input.secondaryColor === undefined ? current.secondaryColor : input.secondaryColor?.toLowerCase() || null,
          colorName: input.colorName === undefined ? current.colorName || null : input.colorName?.trim().slice(0, 80) || null,
          rating: input.rating === undefined ? Number.isInteger(current.rating) ? current.rating : 0 : input.rating,
          feedbackTags: input.feedbackTags === undefined ? Array.isArray(current.feedbackTags) ? current.feedbackTags : [] : [...new Set(input.feedbackTags)].slice(0, 6),
          feedbackNote: input.feedbackNote === undefined ? current.feedbackNote || "" : input.feedbackNote?.trim().slice(0, 500) || "",
        };
        updated.palette = [updated.color, updated.secondaryColor].filter(Boolean);
        const next = [...records];
        next[index] = updated;
        await atomicJson(importedFile, next);
  ...
  return {
    ...
    async configResolved(config) {
      ...
      outfitFeedbackFile = path.join(dataDir, "outfit-feedback.json");
